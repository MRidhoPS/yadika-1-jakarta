...
export default function ContactSection() {
    ...
    const handleSubmit = (e) => {
        ...
        if (isMobile) {
            // MOBILE: Force WhatsApp mobile app to open (not web)
            const mobileAppURL = `whatsapp://send?phone=6281378888032&text=${encodeURIComponent(messageText)}`;
            window.location.href = mobileAppURL;

        } else {
            // DESKTOP/LAPTOP: Open WhatsApp Web
            const webURL = `https://web.whatsapp.com/send?phone=6281378888032&text=${encodeURIComponent(messageText)}`;
